...
This project uses the Notion analysis as a design reference for information architecture, whitespace, card hierarchy, readable typography, and workspace-oriented interactions. It does not copy Notion's brand identity.

- Keep the existing 智汇校园 visual identity: light academic surfaces, blue-green primary accents, calm contrast, and Chinese campus terminology.
- Apply the design system to existing React/Vite/Tailwind components; do not add a new UI framework or runtime dependency just to follow this document.
- Prioritize the role selector, teacher workspace, student learning space, and interactive-book reading room as separate optimization slices.
- Preserve existing role boundaries, same-origin API access, accessibility labels, responsive behavior, and functional workflows while changing visual presentation.
...
- 桌面采用左章节目录、中央开放书页、右 AI 页面问答三栏结构；底部学习航线持续呈现教材闭环与下一任务。
- 开放书页不是静态插画：标题、内容块、练习、提示、笔记和完成状态全部绑定 DeepTutor 真实页面数据。
- 书页以蓝色光轨、中央书脊和克制的星空背景延续学生端视觉；正文对比度和滚动阅读优先于装饰。
- 教材书架收纳在左栏，创建主题教材作为可展开次级操作，不抢占阅读首屏。
- 窄屏改为目录、阅读、问答的纵向流程；章节使用横向滚动，不压缩为不可点击的小字。
